...
const Pokemon: FC<Props> = ({ id }) => {
  ...
  return (
	...
	<div className="statBlock">
		<p className="namecard">Name :{stats.name}</p> 
	  	<p>{stats.HP[0]} :{stats.HP[1]} </p>
		<p>{stats.Attack[0]} :{stats.Attack[1]} </p>
		<p>{stats.SpAttack[0]} :{stats.SpAttack[1]} </p>
		<p>{stats.Spdefense[0]} :{stats.Spdefense[1]} </p>
		<p>{stats.speed[0]} :{stats.speed[1]} </p>
